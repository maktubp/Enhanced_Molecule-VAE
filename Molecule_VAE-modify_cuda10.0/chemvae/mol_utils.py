...
def fix_smiles(smiles:str):
    smiles = smiles.replace(' ','')  # 删除空格
    smiles = smiles.replace('c','C')  # 芳香性毕竟是少数
    smiles = smiles.replace("()",'')  # 删除多余的空括号

    # 修改环编号
    if smiles.count('1') % 2 != 0:
        smiles = smiles.replace('1', '', 1)
    if smiles.count('2') % 2 != 0:
        smiles = smiles.replace('2', '', 1)
    ...
    smiles = remove_parentheses(smiles)
    ...
    def add_parentheses(s):
        ...
        index = 0
        while index < len(s)-2:
            if s[index] == '=' and s[index+1] == 'O' and (s[index-1] != '(' or s[index+2] != ')' ):
                s.insert(index, '(')
                s.insert(index+3, ')')
            index += 1
        return("".join(s))

    smiles = add_parentheses(smiles)
    ...
    def fix_nitrile(s):
        ...
        index = 0
        while index < len(s)-2:
            if s[index] == 'C' and s[index+1] == 'N':
                s.insert(index, '(')
                s.insert(index+2, '#')
                s.insert(index+4, ')')
            elif s[index] == 'N' and s[index+1] == 'C':
                s.insert(index, '(')
                s.insert(index+2, '#')
                s.insert(index+4, ')')
            index += 1
