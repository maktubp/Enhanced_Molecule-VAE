...
import pandas as pd
import os
from .mol_utils import fast_verify, fix_smiles
...
class VAEUtils(object):
    ...
    def estimate_estandarization(self):
        ...
        for chunk in self.chunks(list(range(len(smiles))), batch):
            ...
            one_hot = self.smiles_to_hot(sub_smiles, canonize_smiles=False)
    ...
    def prep_mol_df(self, smiles, z):
        ...
        df['smiles'] = df['smiles'].apply(fix_smiles)  # 大大提高了生成的有效性
        df = df[df['smiles'].apply(fast_verify)]  # 检查SMILES的语法，检查括号匹配和环是否闭合
        if len(df) > 0:
            df['mol'] = df['smiles'].apply(mu.smiles_to_mol)  # SMILES生成mol格式，检查SMILES的合理性
        if len(df) > 0:
